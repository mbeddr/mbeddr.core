...
public abstract class BaseWizard<TResult> extends AbstractWizardEx {
  ...
  private static List<? extends AbstractWizardStepEx> applyExtension(final String myId, List<? extends AbstractWizardStepEx> originalSteps) {
    ...
    Iterable<IWizardExtension> myExtensions = Sequence.fromIterable(new ExtensionPoint<ExtensionProvider>("com.mbeddr.mpsutil.process.runtime.wizardExtensions").getObjects()).translate((it) -> it.getExtensions()).where((it) -> it.forWizard().equals(myId));
  ...
  public BaseProcessStep getStep(final String id) {
  ...
  public BaseProcessStep<TResult> getNextStep(BaseProcessStep current) {
  ...
  public BaseProcessStep<TResult> getPrevStep(BaseProcessStep current) {
    int i = ListSequence.fromList(mSteps).indexOf(current);
