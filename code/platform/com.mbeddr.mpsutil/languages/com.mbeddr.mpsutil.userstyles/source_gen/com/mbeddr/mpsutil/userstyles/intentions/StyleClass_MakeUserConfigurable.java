...
public class StyleClass_MakeUserConfigurable extends IntentionsFactory {
  ...
  public void execute(SNode node, EditorContext editorContext) {
    if ((new IAttributeDescriptor.NodeAttribute(CONCEPTS.UserConfigurable$b0).get(node) == null)) {
      new IAttributeDescriptor.NodeAttribute(CONCEPTS.UserConfigurable$b0).setNew(node);
    } else {
      SNodeOperations.deleteNode(new IAttributeDescriptor.NodeAttribute(CONCEPTS.UserConfigurable$b0).get(node));
    }
  }
  public String description(SNode node, EditorContext editorContext) {
    return ((new IAttributeDescriptor.NodeAttribute(CONCEPTS.UserConfigurable$b0).get(node) == null) ? "Make User Configurable" : "Remove User Configurable");
  }
