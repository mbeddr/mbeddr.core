...
public class ReplHelper {
  ...
  public static boolean isNodeDotGen(SNode expr) {
  ...
  public static String labelName(SNode expr) {
    if (!(SNodeOperations.isInstanceOf(expr, CONCEPTS.DotExpression$yW))) {
      return null;
    }
    SNode call = ListSequence.fromList(SNodeOperations.getNodeDescendants(expr, CONCEPTS.Node_ConceptMethodCall$mz, false, new SAbstractConcept[]{})).first();
  ...
  public static void createResolveStatement(SNode expr, final String labelName, SNode mc) {
    SNode inputNode = SLinkOperations.getTarget(SNodeOperations.cast(expr, CONCEPTS.DotExpression$yW), LINKS.operand$w6IR);
    ...
    SNodeOperations.replaceWithAnother(expr, createDotExpression_dx7ny_a0a2a11(inputNode, label));
  }

  private static SNode createDotExpression_dx7ny_a0a2a11(SNode p0, SNode p1) {
    SNodeBuilder n0 = new SNodeBuilder().init(CONCEPTS.DotExpression$yW);
    {
      SNodeBuilder n1 = n0.forChild(LINKS.operation$gs9E).init(CONCEPTS.GenerationContextOp_GetOutputByLabelAndInput$Kl);
      n1.forChild(LINKS.inputNode$VGCv).initNode(p0, CONCEPTS.Expression$mB, true);
      n1.setReferenceTarget(LINKS.label$VAL6, p1);
    }
    n0.forChild(LINKS.operand$w6IR).init(CONCEPTS.TemplateFunctionParameter_generationContext$Yo);
    return n0.getResult();
  }
