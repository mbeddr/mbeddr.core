    ...
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (SNodeOperations.hasRole(node, LINKS.iterator$JLmf)) {
          SNode forStatement = SNodeOperations.getNodeAncestor(node, CONCEPTS.ForStatement$IY, false, false);
          List<SNode> additionalIterators = SLinkOperations.getChildren(forStatement, LINKS.additionalIterators$YBJx);

          if (ListSequence.fromList(additionalIterators).isNotEmpty()) {
            SNode firstIterator = SNodeOperations.cast(ListSequence.fromList(additionalIterators).first(), CONCEPTS.ForVarRef$AT);
            SNode copyIterator = SNodeOperations.copyNode(firstIterator);
            SNodeOperations.replaceWithAnother(node, copyIterator);
            SNodeOperations.deleteNode(firstIterator);
            SelectionUtil.selectNode(editorContext, SLinkOperations.getTarget(copyIterator, LINKS.init$PAJi));
            return;
          }
        }
