...
public class StructPrintingHashHelper {
        ...
        if (SNodeOperations.isInstanceOf(pt, CONCEPTS.PointerType$HX)) {
          {
            ...
            if (SNodeOperations.isInstanceOf(st, CONCEPTS.StructType$B3)) {
              StructPrintingHashHelper.addAnnotation(st);
            }
          }
        }
      }
  ...
  private static void addAnnotation(SNode st) {
    if ((new IAttributeDescriptor.NodeAttribute(CONCEPTS.GenStructHashPrintFunction$ij).get(SLinkOperations.getTarget(st, LINKS.struct$WCsg)) == null)) {
      new IAttributeDescriptor.NodeAttribute(CONCEPTS.GenStructHashPrintFunction$ij).setNew(SLinkOperations.getTarget(st, LINKS.struct$WCsg));
