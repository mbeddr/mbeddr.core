  ...
  /*package*/ static AbstractCellAction createAction_COMPLETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        EditorCell selected = editorContext.getSelectedCell();
        if (selected == null || selected.getParent() == null) {
          return;
        }
        EditorCell_Collection parent = selected.getParent();
        SubstituteInfo info = parent.getSubstituteInfo();
        if (info == null) {
          return;
        }
        NodeSubstitutePatternEditor pe = ((jetbrains.mps.nodeEditor.cells.EditorCell) selected).createSubstitutePatternEditor();
        NodeSubstituteChooser chooser = ((EditorComponent) editorContext.getEditorComponent()).getNodeSubstituteChooser();
        chooser.setNodeSubstituteInfo(info);
        chooser.setPatternEditor(pe);
        chooser.setContextCell(parent);
        chooser.setIsSmart(false);
        chooser.setContextCell(parent);
