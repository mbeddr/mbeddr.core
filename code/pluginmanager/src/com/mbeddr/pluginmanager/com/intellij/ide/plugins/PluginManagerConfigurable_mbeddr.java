...
public class PluginManagerConfigurable_mbeddr extends BaseConfigurable implements SearchableConfigurable, Configurable.NoScroll {
  public static final String ID = "preferences.pluginManager.mbeddr";
  public static final String DISPLAY_NAME = IdeBundle.message("title.plugins");
  private static final String MBEDDR_REPOSITORY_URL = "http://mpsrepo.slisson.de/";

  protected final PluginManagerUISettings_mbeddr myUISettings;

  private PluginManagerMain myPluginManagerMain;
  private boolean myAvailable;

  public PluginManagerConfigurable_mbeddr(final PluginManagerUISettings_mbeddr UISettings) {
    myUISettings = UISettings;
  }
  
  public PluginManagerConfigurable_mbeddr(final PluginManagerUISettings_mbeddr UISettings, boolean available) {
    myUISettings = UISettings;
  ...
  public JComponent createComponent() {
    registerMbeddrRepository();
    return getOrCreatePanel().getMainPanel();
  }

  public void registerMbeddrRepository() {
    List<String> hosts = UpdateSettings.getInstance().getStoredPluginHosts();
    if (!hosts.contains(MBEDDR_REPOSITORY_URL)) {
      hosts.add(MBEDDR_REPOSITORY_URL);
    }
  }
