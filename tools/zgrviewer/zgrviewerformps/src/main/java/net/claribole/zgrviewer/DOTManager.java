...
class DOTManager {
	...
	void load(File f, short prg, boolean parser) {
		...
		try {
			svgF = Utils.createTempFile(ConfigManager.m_TmpDir.toString(),
					"zgrv", (parser ? ".dot" : ".svg"));
			dotF = f;
	...
	private void callGraphViz(short prg, boolean parser) throws Exception {
		// prg is the program to use DOTManager.*_PROGRAM
		try {
			grMngr.gp.setMessage("Preparing "
					+ (parser ? "Augmented DOT" : "SVG") + " Temp File");
			grMngr.gp.setProgress(10);
			if (parser) {
				if (!generateDOTFile(dotF.getAbsolutePath(),
						svgF.getAbsolutePath(), prg)) {
					deleteTempFiles();
					return;
				}
				displayDOT();
				...
				if (!generateSVGFile(dotF.getAbsolutePath(),
						svgF.getAbsolutePath(), prg)) {
					deleteTempFiles();
					return;
				}
	...
	protected String getProgram(short prg) {
		...
		String osName = System.getProperty(ConfigManager.OS_ENV_ID)
				.toLowerCase();
		if (osName != null) {
			if (osName.contains(ConfigManager.MAC_OS_NANME)
					|| osName.contains(ConfigManager.LINUX_OS_NAME))
	...
	private boolean generateDOTFile(String dotFilePath, String tmpFilePath,
			short prg) {
		...
		if (programFile.exists()) {
			...
			System.err
					.println("Please install graphiz an make sure that the \"dot\" program is located in: "
							+ ConfigManager.XOS_GRAPHVIZ_BASEDIR);
			return false;
		}
		return true;
	}
	 ...
	 * 
	 * @param dotFilePath
	 *            the name of the DOT data file
	 * @param svgFilePath
	 *            the name of the output data file
	 * @param prg
	 *            program to use (dot or neato)
	 * @return true if success; false if any failure occurs
	 */
	private boolean generateSVGFile(String dotFilePath, String svgFilePath,
			short prg) {
		String[] cmdArray = new String[(cfgMngr.FORCE_SILENT) ? 7 : 6];
		...
		try {
			try {
				File execDir = (new File(dotFilePath)).getParentFile();
				System.out.println("executing " + cmdArray[0] + " "
						+ cmdArray[1] + " " + cmdArray[2] + " " + cmdArray[3]
						+ " " + cmdArray[4] + " " + cmdArray[5] + " "
						+ " in dir:" + execDir.getAbsolutePath());
			...
			String error = 
				"Error: generating OutputFile.\n" +
				"Maybe no \"Dot\" program found.\n" +
		        "Please install graphiz and make sure that the \"dot\" program is on path (Windows) or located in " +
		        ConfigManager.XOS_GRAPHVIZ_BASEDIR + "(Linux, Mac)";
				JOptionPane.showMessageDialog(grMngr.mainViewPanel, error);	
			return false;
		}
	...
	protected void executeProcess(Process p) throws InterruptedException,
			IOException {
	...
	void loadCustom(String srcFile, String cmdLineExpr) {
		...
		System.out.println(".......---->" + ConfigManager.m_TmpDir.toString());
		try {
			svgF = Utils.createTempFile(ConfigManager.m_TmpDir.toString(),
					"zgrv", ".svg");
			if (!generateSVGFileFOP(srcFile, svgF.getAbsolutePath(),
					cmdLineExpr)) {
				deleteTempFiles();
				return;
			}
			...
			grMngr.gp.setVisible(false);
			javax.swing.JOptionPane.showMessageDialog(
					grMngr.mainView.getFrame(), Messages.loadError + srcFile);
		}
	}
	 ...
	 * @return true if success; false if any failure occurs
	 */
	private boolean generateSVGFileFOP(String srcFilePath, String svgFilePath,
			String commandLine) {
		...
		try {
			...
			JOptionPane.showMessageDialog(grMngr.mainView.getFrame(),
					Messages.customCallExprError2 + Utils.join(cmdArray, " "),
	...
	void displaySVG(File sourceDotFileParentDir) {
		...
		try {
			...
			SVGReader.load(svgDoc, grMngr.mSpace, true, svgF.toURI().toURL()
					.toString(), sourceDotFileParentDir.toURI().toURL()
					.toString());
			grMngr.seekBoundingBox();
			grMngr.buildLogicalStructure();
		} catch (MalformedURLException ex) {
			JOptionPane.showMessageDialog(grMngr.mainView.getFrame(),
					svgF.getAbsolutePath(), Messages.SVG_PARSING_ERROR,
	...
	void displayDOT() throws Exception {
		try {
			...
			grMngr.gp.setProgress(60);
			DataInputStream graphInput = new DataInputStream(
					new FileInputStream(svgF));
			...
			JOptionPane.showMessageDialog(grMngr.mainView.getFrame(),
					Messages.ERROR_LOADING_DOT_FILE, "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
...
/**
 * A simple thread that will consume the stdout and stderr streams of a process,
...
class ProcessOutputConsumer extends Thread {
	...
	/**
	 * @param p
	 *            The process whose stdout and stderr streams are to be
	 *            consumed.
	 * @throws IOException
	 */
	 ...
	 *            consumed.
	 * @param waitTime
	 *            How long to wait (in ms) between checks for output to consume.
	 * @throws IOException
	 */
